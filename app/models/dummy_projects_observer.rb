class DummyProjectsObserver < ActiveRecord::Observer
  ...
  def after_create(user)
    projects = user.projects.create([
      ...
      {title: 'Make my wife happy', started_at: Date.today, finish_at: 11.days.since, color_index: 2},
      {title: 'Start my own business', started_at: Date.today, finish_at: 12.days.since, color_index: 3},
    ])
    p = projects.first
    p.events.create([
      {title: 'By a book', date: Date.tomorrow, time: Time.current},
    ...
    p = projects[1]
    p.events.create([
      {title: 'Buy a present', date: Date.tomorrow, time: Time.current},
      {title: 'Go shopping together', date: 10.days.since, time: Time.current}
    ])
    p = projects.last
    p.events.create([
      {title: 'Think about the idea', date: Date.tomorrow, time: Time.current}
    ])
  end
end
